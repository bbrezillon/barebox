				       ...
				       const struct usb_clk_data *data)
{
	...
	for_each_set_bit(i, (unsigned long *)&data->clk_mask,
		...
		of_property_read_string_index(node, "clock-output-names",
					      j, &clk_name);
		clk_data->clks[i] = clk_gate(clk_name, clk_parent, reg,
					     i, 0, 0);
	...
	reset_data->reg = reg;
	reset_data->rcdev.nr_resets = fls(data->reset_mask) + 1;
...
static int __init sun4i_a10_usb_setup(struct device_node *node)
{
	sunxi_usb_clk_setup(node, &sun4i_a10_usb_clk_data);
	return 0;
}
...
static int __init sun5i_a13_usb_setup(struct device_node *node)
{
	sunxi_usb_clk_setup(node, &sun5i_a13_usb_clk_data);
	return 0;
}
...
static int __init sun6i_a31_usb_setup(struct device_node *node)
{
	sunxi_usb_clk_setup(node, &sun6i_a31_usb_clk_data);
	return 0;
}
...
static int __init sun8i_a23_usb_setup(struct device_node *node)
{
	sunxi_usb_clk_setup(node, &sun8i_a23_usb_clk_data);
	return 0;
}
...
static int __init sun9i_a80_usb_mod_setup(struct device_node *node)
{
	sunxi_usb_clk_setup(node, &sun9i_a80_usb_mod_data);
	return 0;
}
...
static int __init sun9i_a80_usb_phy_setup(struct device_node *node)
{
	sunxi_usb_clk_setup(node, &sun9i_a80_usb_phy_data);
	return 0;
}
